...
```{r}
ggplot(co2, aes(x = decimal_date, y = CO2concentration)) + 
  geom_line() + 
...
carbon <-co2 |> 
  group_by(month) |> 
  summarise(avgM = mean(CO2concentration))


plot <- ggplot(carbon, aes(x = month, y = avgM)) + 
  geom_point()
...
**Answer 1:** The trend in the CO2 data is upward trending and represents a positive correlation of CO2 concentration and year.
...
CO2 start off a little above 360 then peak at the start of summer, decline through October, then spike back up around 360 at the end of the year.

It is max in may and min in October. Human co2 burn might be higher in the earlier months because they are more active.
...
Each column is a numeric class and the no_smoothing column and lowess are measured in celcius

Global temperature is the actual recorded temp

Global temperature anomaly is the actual global temp subtracted by the long term average.
...
```{r}
dat <- read_table("https://data.giss.nasa.gov/gistemp/graphs/graph_data/Global_Mean_Estimates_based_on_Land_and_Ocean_Data/graph.txt",
                   skip = 5,
...
```{r}
ggplot(dat, aes(x = Year, y = AnnualMean)) + 
  geom_point() + 
  labs(x = "Year", y = "Global Mean Temps")
```

The plot shows the Global Mean temperature has steadily increased over time. It seems the average temp has spiked twice, likely due to heavy human movements.
...
I searched calculate rolling means in R

Website that helped: <https://www.geeksforgeeks.org/how-to-calculate-a-rolling-average-in-r/>

I needed to download more libraries and write a new line of code that calculates the rolling average. I had to switch the k to 1.
...
dat$Rolling_Avg <- rollmean(dat$AnnualMean, k = 1, fill = NA, align = "right")
...
A 5 year average is the average of 5 consecutive years while an annual average is the mean of all the years.

```{r}
Run <- dat |> 
  mutate(year5 = rollmean(dat$AnnualMean, k = 5, fill = NA, align = "right"), year10 = rollmean(dat$AnnualMean, k = 10, fill = NA, align = "right"), year20 = rollmean(dat$AnnualMean, k = 20, fill = NA, align = "right"))
```
...
```{r}
ggplot(Run, aes(x = Year)) +
  geom_line(aes(y = AnnualMean, color = "Annual Average"), alpha = 0.5) +
  geom_line(aes(y = year5, color = "5-Year Average"), size = 1) +
  geom_line(aes(y = year10, color = "10-Year Average"), size = 1) +
  geom_line(aes(y = year20, color = "20-Year Average"), size = 1) + 
  labs(y = "Averages")
```
Chat GPT was used to teach me how to lay each average line on the same graph, and to color them differently.
...
Looking at the plot, there does not seem to be much of a pause in Average CO2 emission since 1998. In fact all the averages seem to be increasing more.
...
The data set contains 4 columns which include the depth in meters, age of ice in (yr BP), age of air in (yr BP), and co2 concentration in ppvm.

The temporal resolution of the data is from 363 observations of different depths of Ice recorded between 4 climate cycles in january 1998. The deeper the ice is, the older the air and ice are.

```{r}
vostok <-  read_table("vostok.icecore.co2", 
                  skip = 20,
                  col_names = c("Depth(m)", "IceAge", "AirAge",
                                "CO2concentration"))
Ice <- vostok |> 
  mutate(Age = IceAge - AirAge) |> 
  arrange(desc(Age)) |> 
  mutate(Year = 2025 - Age) 
 


ggplot(Ice, aes(x = Year, y = CO2concentration)) + 
  geom_point() + 
  labs(x = "Age (- = BC)", y = 'CO2 concentration (ppvm)')
  
```

```{r}
final <- bind_rows(Ice, co2)

ggplot(final, aes(x = Year, y = CO2concentration)) + 
  geom_point() 
  
```

Recent Co2 observations compared to historical observations are very different. There has always been a gradual increase in co2 from around 4000 BC to 100 AC. The data from 1958 to 2025 indicates an almost vertical increase compared to the historical increase. It seems much more human activity has occurred in those 70 years that the rest of history.
